...
void MultiplexerCrossbar::constructModel()
{
    ...
    for (unsigned int cur_input = 0; cur_input < number_inputs; cur_input++) {
        createElectricalEventResult("DinMulticastInv" + (String)cur_input);
        EventInfo* event_info = getEventInfo("DinMulticastInv" + (String)cur_input);
        // Assuming In0 send data to all Outputs
        for (unsigned int i = 0; i < number_inputs; i++) {
            if (i == cur_input) {
                //set transitions at current input pin to be (0.25,0.25,0.25,0.25)->(00,01,10,11)
                event_info->setEqualDynamicTransitionInfo("In" + (String)i);
            } else {
                //set all other inputs Transition info to be 1,0,0,0->(00,01,10,11)
                event_info->setTransitionInfo0("In" + (String)i);
            }
        }
        //set Transition Info of select Signals
        for (unsigned int cur_output = 0; cur_output < number_outputs; cur_output++) {
            // we will have total log2(number_input) stages, based on input binary representation
            // select_signla stage will be either zero or 1 , number_selects for each output
            unsigned int quo = cur_input;
            for (unsigned int cur_stage = 0; cur_stage < number_selects; cur_stage++) {
                if (quo % 2 == 0) {
                    //select signal transition probabilities to 0.5,0,0.5,0->(00,01,10,11)
                    event_info->setTransitionInfo00(String::format("Sel%d_%d", cur_output, cur_stage));
                } else {//select signal transition probabilities to 0,0.5,0,0.5->(00,01,10,11)
                    event_info->setTransitionInfo11(String::format("Sel%d_%d", cur_output, cur_stage));
                }
                quo = quo / 2;
            }
        }
