...
void EventInfo::setTransitionInfo0(const String& port_name_)
{
    ASSERT(m_trans_info_map_->keyExist(port_name_), "[Error] " + getEventName() +
           " -> Port (" + port_name_ + ") does not exist!");

    m_trans_info_map_->set(port_name_, TransitionInfo(1, 0, 0, 0));
    return;
}

void EventInfo::setTransitionInfo00(const String& port_name_)
{
    ASSERT(m_trans_info_map_->keyExist(port_name_), "[Error] " + getEventName() +
           " -> Port (" + port_name_ + ") does not exist!");

    m_trans_info_map_->set(port_name_, TransitionInfo(0.5, 0, 0.5, 0));
    return;
}


void EventInfo::setTransitionInfo11(const String& port_name_)
{
    ASSERT(m_trans_info_map_->keyExist(port_name_), "[Error] " + getEventName() +
           " -> Port (" + port_name_ + ") does not exist!");

    m_trans_info_map_->set(port_name_, TransitionInfo(0, 0.5, 0, 0.5));
    return;
}

void EventInfo::setEqualDynamicTransitionInfo(const String& port_name_)
{
    ASSERT(m_trans_info_map_->keyExist(port_name_), "[Error] " + getEventName() +
           " -> Port (" + port_name_ + ") does not exist!");

    m_trans_info_map_->set(port_name_, TransitionInfo(0.25, 0.25, 0.25, 0.25));
    return;
}
